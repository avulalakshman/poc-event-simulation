...
public class PocEventSimulationApplication implements CommandLineRunner {
	...
	private void initData() {
		List<Service> list = serviceUtil.getServices();
	...
	void getDetailsOf(List<Service> services) throws IOException {
		...
		do {
			...
			for (Service s : services) {
				System.out.println(String.format("\t\t %d) %s", i++, s.getSvcName()));
			}
	...
	private List<DimensionData> getDimensionsInput(Scanner scanner, Service svc) {
		...
		for (ServiceDimension sd : svcDimensions) {
			...
			case INPUT:
				...
				userInput.add(DimensionData.builder().dimensionName(sd.getSvcDimName()).dimensionValue(val).build());
				break;
			case PICKLIST:
				System.out.println("\n\t" + sd.getDescr());
				...
				if (choice >= 0 && choice < sd.getPicklist().size()) {
					userInput.add(DimensionData.builder().dimensionName(sd.getSvcDimName())
							.dimensionValue(sd.getPicklist().get(choice).getName()).build());
				}
				break;
			case DERIVED:
				continue;
